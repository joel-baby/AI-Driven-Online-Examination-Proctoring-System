        ...
        try:
            ...
            if output_format.lower() == 'pdf':
                try:
                    ...
                    self.logger.warning(f"PDF generation failed ({str(pdf_error)}), falling back to HTML")
            ...
            # Try to save at least an HTML version
            try:
                timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
                fallback_path = os.path.join(self.output_dir, f"report_fallback_{timestamp}.html")
                with open(fallback_path, 'w', encoding='utf-8') as f:
                    f.write(f"<html><body><h1>Error generating report</h1><p>{str(e)}</p></body></html>")
                self.logger.info(f"Fallback report saved at: {fallback_path}")
